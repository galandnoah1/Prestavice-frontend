...
import { Menu, X, Hammer, ChevronDown, LayoutDashboard, LogOut } from 'lucide-react'
          ...
          {!user ? (
            ...
            <div className="navbar-user">
              <button className="navbar-user-trigger" onClick={() => setMenuOpen((v) => !v)}>
                <img src={user.photo} alt={user.username} className="navbar-user-avatar" />
                <span>{user.username}</span>
                <ChevronDown size={16} />
              </button>
              {menuOpen && (
                <div className="navbar-user-menu">
                  <Link to={user.role === 'artisan' ? '/dashboard-artisan' : '/dashboard-client'} onClick={() => setMenuOpen(false)}>
